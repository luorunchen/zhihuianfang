(function ($) {
    ...
    $.fn.OpenMap = function () {
        ...
        mapObj = new BMap.Map(mapBox, { enableMapClick: true});
        mapObj.addControl(new BMap.MapTypeControl({ mapTypes: [BMAP_NORMAL_MAP, BMAP_HYBRID_MAP], offset: new BMap.Size(10, 10) }));
        mapObj.centerAndZoom(point,8);
        mapObj.enableScrollWheelZoom(true);
	

        // mapObj.setMapStyle({ style: "googlelite" });
    ...
    $.fn.MapWhere = function (ttext, ttype, tvalue) {
       // console.log(ttext);   //点的哪个一个名字  告警~ 在线~ 离线 ~ 到期
       // console.log(ttype);    //GasDevice
       // console.log(tvalue);    //LstDevice01Company
        var isHas = false;
        $(".MapFilter").find(".fItem").each(function (i, v) {
          //  console.log(v);
            var tobj = $(v);
