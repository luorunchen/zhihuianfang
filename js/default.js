...
/*选择窗体*/
var yingBackFun = null;
var yingGridID = null;
function OpenYingFrame(url, editWidth, editHeight, winID, editTitle, gridID, backFun, modelcss) {
    ...
    var tModel = $('#yingModal');
    console.log(tModel)
    tModel.removeClass();
    tModel.addClass("modal fade");
    tModel.attr('data-backdrop',false)
    tModel.attr('data-keyboard',false)
    if (modelcss) {
        tModel.addClass(modelcss);
    }

    $('#yingModalLabel').text(editTitle);

    var tDialog = $('#yingModalDialog');
    tDialog.width(editWidth);

    var tIfm = $('#yingModelIfm');
    tIfm.attr("src", url);
...
function CloseYingWin(v) {
    var tModel = $('#yingModal');
    tModel.modal('hide');

    if (yingBackFun) {
        yingBackFun(v, yingGridID);
    }
    yingBackFun = null;
    $('#yingModelIfm').attr("src", "about:blank");
}
/*选择窗体*/
