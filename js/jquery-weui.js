  ...
  $.login = function(text, title, onOK, onCancel, username, password) {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p>' +
            '<label>用户名:</label><input type="text" class="weui-input weui-prompt-input" id="weui-prompt-username" value="' + (config.username || '') + '" placeholder="输入用户名" />' +
            '<label>手机号:</label><input type="test" class="weui-input weui-prompt-input" id="weui-prompt-tel" value="' + (config.tel || '') + '" placeholder="输入手机号码" />'+
		    '<label>备&nbsp;&nbsp;&nbsp;注:</label><input type="test" class="weui-input weui-prompt-input" id="weui-prompt-beizhu" value="' + (config.beizhu || '') + '" placeholder="输入备注" />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
        ...
        onClick: function() {
          ...
          var tel = $("#weui-prompt-tel").val();
		  var beizhu = $("#weui-prompt-beizhu").val();
          if (!config.empty && (username === "" || username === null)) {
            modal.find('#weui-prompt-username').focus()[0].select();
            return false;
          }
          if (!config.empty && (tel === "" || tel === null)) {
            modal.find('#weui-prompt-tel').focus()[0].select();
            return false;
          }
          $.closeModal();
          config.onOK && config.onOK.call(modal, username, tel,beizhu);
        }
      }]
    }, function () {
