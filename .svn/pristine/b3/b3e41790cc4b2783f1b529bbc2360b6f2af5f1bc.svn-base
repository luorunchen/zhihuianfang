$(function(){
  ...
  function edit(pno,pageSize){
    ...
    $.ajax({
      ...
      success:function(res){
        ...
        var deviceNumber=res.deviceNumber
        var html="";
        html="该账号下共有<span style='color:red'>"+deviceNumber+"</span>台设备";
        $('#total').html(html);
        var html="";
        if(udata!=""||udata!=null){
          ...
          $('.lookDetail').click(function(){
            ...
            sessionStorage.setItem('comName',comName);
             window.top.OpenFrame("../lingdong/All_ElecComDetail.html?ComID=" + comid + "&ComName=" + comName, 930, 620, "SElect", comName, "", function (r) {
  ...
  function onClick(event, treeId, treeNode, clickFlag){
    ...
    function ShaBefore(pno,pageSize){
      $.ajax({
        type:'get',
        data:'region='+code+'&username='+username+'&pno='+pno+'&pageSize='+pageSize,
        url: url+'/region/getRegionList.action',
        error:function(error){console.log(error)},
        success:function(res){
          console.log(res);
          ...
          var deviceNumber=res.deviceNumber
          var html="";
          html="当前查看的地区是<span style='color:red'>"+regionName+"</span>,共有"+deviceNumber+"台设备";
          $('#total').html(html);
          var html="";
          if(res.mess.length!=0){
            ...
            console.log(1);
            html="<tr><td class='td_center' colspan='8'>无数据</td></tr>";
            $('#GridTables').html(html)
          }
  ...
  function daoFac(code,reName){
    sessionStorage.setItem('daoCode',code)
    sessionStorage.setItem('daoreName',reName)
  }
  daoFac(1,'');

  $('.daochuFac').click(function(e){
    e.preventDefault();
      var width = $(window).width();
      var height = $(window).height();
      var code=sessionStorage.getItem('daoCode')
      var reName=sessionStorage.getItem('daoreName')
      layer.open({
        type: 2,
        title: '导出设备表',
        maxmin: true,
        ...
        content: "../Sys/SUser/daoFacBiao.html?code="+code+'&reName='+reName+'&username='+username
        // content: "../Sys/SUser/daoFacBiao.html?code="+code+'&reName='+reName+'&username='+username+'&type='+type+'&online='+online+'&alarm='+alarm
    });
