...
 	})();
            ...
            //  console.log(udata);
                ...
                $('.deleClose').click(function(){
                    ...
                    if(powerId == 1000) {
                        $.confirm({
                            title: ',确定删除此项目的分享吗',
                            text: "删除设备名为<span style='color:red;'>"+name+"</span>的记录",
                            onOK: function () {
                                $.ajax({
                                    type:'get',
                                    url:url+'/WebProject/deleteSharedevice.action',
                                    data:'deviceId='+devId +'&my_username='+username+'&username='+global.username,
    ...
    $('.newAdd').click(function(){
        ...
        layer.open({
            ...
            shadeClose: true, //点击遮罩关闭层
            shade: 0.5, //不显示遮罩
            area: [width*0.95 + 'px', height-50 + 'px'],
            content: "./user_facadmin_share.html?username="+global.username
        });
    })
     //刷新
