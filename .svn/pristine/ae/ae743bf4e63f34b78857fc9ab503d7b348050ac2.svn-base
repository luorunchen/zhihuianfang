   ...
   var panoramaService = new BMap.PanoramaService();   //3D场景服务
    ...
    var loadData = function () {
       ...
       // console.log(type+'---'+userName);
        doAction(anfang_url+"/WebProject/DeviceProject.action?username="+userName, "GET", {type:type,region:region}, function (result) {  
            gLastResult = result;
            console.log(result);
    ...
    var loadAalarm = function () {
        ...
        doAction(anfang_url+"/WebProject/DeviceAlarm.action?username="+userName, "GET", {type:type,region:region}, function (result) {
            console.log(result);
            if(result.length==0){
                var html="";
    ...
    var loadNum = function () {
        doAction(anfang_url+"/WebProject/DeviceNum.action?username="+userName, "GET", {type:type,region:region}, function (result) {
            console.log(result);
            var region=localStorage.getItem('region')
            if(result.length!=0){
                var DevAll = 0;
                $("#Dev01").html(result.alarm);
                if(userName=='13076920054' || region=='1'){
                    $("#Dev02").html(Number(result.online)+20000);
                }else{
                    $("#Dev02").html(Number(result.online));
                }
                ...
                if(userName=='13076920054' ){
                    DevAllStr = "000000" + Number(DevAllStr+20000);
                }else{
                    DevAllStr = "000000" + Number(DevAllStr);
                }
    ...
    var openCom = function (comid,comName) {
        ...
        window.top.OpenFrame("./SElec_ElecComDetail.html?ComID=" + comid + "&ComName=" + comName, 930, 600, "SElect", comName, "", function (r) {
