function MapTool(box, map,mData) {
    ...
    var loadEvent = function () {
        ...
        $(".queryBut").click(function () {
            ...
            $.each(lstCompany, function (i, v) {
                if (v.name.indexOf(txt) >= 0 || v.introduce.indexOf(txt) >= 0 || v.location.indexOf(txt) >= 0) {
                    var thtml = "<div class=\"resultItem\">";
                    thtml += "<div class=\"resultTitle\">" + v.name + "</div>";
                    thtml += "<div class=\"resultText\">地址：" + v.location + "</div>";
                    if (v.master && v.master != "") {
                        thtml += "<div class=\"resultText\">电话：" + v.master + "</div>";
                    }
                    thtml += "</div>";
    ...
    var CompanyClick = function (pobj, data) {
        ...
        pobj.click(function () {
            if (data){
                var cc=data.long_latbai;
                var dd=cc.split(',');
                var point = new BMap.Point(dd[1], dd[0]);
